...
`RoomService` (`Server/src/application/room-service.ts`, ~2670 lines) is the core — it holds all rooms and connections in memory and routes every incoming command. All state is **in-memory only**; no database. `runHousekeeping()` runs on a 1-second interval for idle room cleanup and questioner-reconnect timeout.
