...
房间 ID `Oblivionis`（大小写不敏感）为专用测试与调试房间。与受限的普通房间不同，该房间提供调试控制台（TestController），拥有自由跳转阶段、快速跳过倒计时、注入与移除测试人机、强制指派角色的调试控制能力，所有操作均发送真实服务端命令。跳转到需要完整人数的阶段前，应先用机器人补满房间；对局中途加入的机器人会成为观战者。
