...
- **选区前后延展与音频渐入渐出**：题目音频选区起点前置 1 秒（`Math.max(0, windowStartTime - 1000)`）、终点后延 1 秒（`min(duration, maxLineEnd + 1000)`）。客户端通过 `useAudioClipPlayer` 在首个 1 秒内执行音量由 0 到 1 的平滑渐入，在终点前最后 1 秒内执行由 1 到 0 的平滑渐出，彻底消除音频启停爆音与突然掐断感。
