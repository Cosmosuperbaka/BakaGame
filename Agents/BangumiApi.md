...
自动出题支持年份范围、总榜/年榜排名范围、网易云歌曲热度和歌曲类型筛选。总榜直接使用 Bangumi
热度排序，年榜会在设置的年份范围内先随机选择一个年份，再按该年份的热度排序取候选作品。
候选条目按顺序尝试，
歌曲热度不达标时优先继续尝试同一作品的其它曲目，只有该作品所有曲目都不可用时才切换作品；
全部失败时返回 `BANGUMI_NO_MUSIC`。结算摘要只公开最多 5 条作品标签、关联歌曲及其
OP/ED/插曲/其它类型，不公开元标签。
