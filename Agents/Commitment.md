...
### 编辑时机与累积覆盖铁律 (Settlement Window)

`changelog.json` 的每一次编辑都是一个**结算点**，必须对上一次结算以来的全部改动负责，而不是只登记触发本次编辑的那一条：

- **起点判定**：用 `git log -1 -- Client/src/data/changelog.json` 取出上一次编辑该文件的提交，本次结算窗口即「该提交之后 → 当前 HEAD」的全部提交。
- **强制回看窗口**：编辑前必须逐个过一遍窗口内的提交，识别出其中所有**用户可见变动**，本期一并写入。窗口内存在多项用户可见变动时，宁可归并在更高语义层级的一条摘要里，也**严禁漏记**。
- **允许窗口为空**：窗口内若确实只有内部改动（重构、测试、CI、部署方式、遥测埋点、命名统一），不新增版本条目，也不得为凑数编造面向玩家的文案。
- **版本递增**：本窗口确有用户可见变动时，按上方版本号表判定 `major` / `minor` / `patch` 并新增条目；无用户可见变动则不得新增版本条目。
- **条目粒度**：仍受「单版本总条目建议 2 ~ 4 条、单个分类 1 ~ 2 条」约束，超出时在本窗口内合并概括，不得把一个改动拆成多条凑字数。
