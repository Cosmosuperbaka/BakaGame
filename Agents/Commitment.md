...
3. **极简精炼铁律（剔除汇报腔与修饰性后置从句）**:
   - **零修饰性尾巴（Zero Rhetoric Suffixes）**：严禁在动宾短语后拖挂任何宣传式、解释性或收益性尾巴（严禁出现“，提升XXX性能/稳定性/体验”、“，大幅缩减/减少XXX”、“，显著加快XXX”、“，消除XXX”、“，并提供XXX控制”）。用户只关注**客观改动**，绝不阅读 AI 自吹自擂的汇报套话。
   - **高维度高度概括（High-Level Synthesis）**：多项同类别局部改动（如多项字体微调、样式调整），必须在更高语义层级提炼为一句干练短语（如直接写“优化字体表现”），严禁把工程拆解细节平铺直叙列成多条长句。
   - **单条字数与结构硬约束（Atomic SVO）**：优先采用纯粹动宾短语（“优化...”、“完善...”、“加固...”、“修复...”），单条长度严格控制在 **6 ~ 25 字** 以内；避免使用连词（“并”、“且”、“从而”）。
   - **单版本条目上限（Strict Entry Cap）**：单版本总条目建议控制在 **2 ~ 4 条**；单个分类一般不超过 **1 ~ 2 条**，保证更新日志弹窗无需滚屏、3 秒内一目了然。

### 4. changelog 正误对照清单 (Good Taste vs Bad Taste)

| 场景 | ❌ 冗长赘述与汇报腔（Bad Taste） | ✅ 极简克制真实客观（Good Taste） |
|---|---|---|
| 字体排版重构 | `重构全站排版字体体系，置顶经典英文衬线与矢量中文，提升文字屏显清晰度与抗锯齿表现` | `优化字体表现` |
| 静态资源转码 | `聊天表情包与全站图片资源支持WebP自动转码，大幅减少流量消耗并加速加载` | `聊天表情包与全站图片资源支持WebP自动转码` |
| 前端路由分包 | `前端页面支持路由级按需动态拆包，显著缩减首屏体积与加载耗时` | `前端页面支持路由级动态拆包` |
| 异常监控体系 | `完善全栈异常捕获与监控体系，提升页面与服务容错能力` | `完善全栈异常捕获与监控体系` |
| 安全韧性加固 | `加固数据通道与遥测通信安全，提升弱网环境下的服务容错与恢复韧性` | `加固数据通道与遥测通信安全` |
| 网络协议优化 | `优化长连接通信与协议解析性能，提升对局稳定性` | `优化长连接通信与协议解析性能` |
| 结算副歌播放 | `Songuessr结算阶段支持自动播放揭晓歌曲副歌片段，并提供播放与重播控制` | `Songuessr结算阶段支持自动播放揭晓歌曲副歌片段` |
