...
- **客户端异常过滤必须覆盖全部浏览器引擎文案**：客户端 `ignoreErrors` 严禁只覆盖 Chromium 文案。模块与分块加载失败必须同时覆盖 Chromium（`Failed to fetch dynamically imported module`、`Loading chunk N failed`）与 WebKit（`Importing a module script failed`）两套文案，否则同一类失败会因引擎措辞不同而漏网。这类失败已由自愈重载兜底，不得重复上报污染真实缺陷信号。
