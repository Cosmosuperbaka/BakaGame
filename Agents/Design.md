...
- 页面主体使用 `background`、`foreground`、`card`、`muted`、`border`、`primary` 等语义色，不以硬编码颜色替代通用语义。
- 基础界面保持白色或中性背景、清晰边框和深色正文；次要区域可使用 `bg-muted/30`、`bg-muted` 或 `text-muted-foreground` 降低层级。
- `primary` 只用于主要操作、当前玩家信息、本人消息或明确的交互焦点，避免大面积铺色。
...
- 沿用系统字体栈：`system-ui, -apple-system, sans-serif`，不得无需求引入网页字体。
...
- 按钮、输入框和徽章使用现有 `rounded-md`；主要面板和内容卡片通常使用 `rounded-xl`；聊天气泡可使用 `rounded-2xl` 并通过单侧小圆角区分消息方向。
- 主要结构通过一层轻边框区分。阴影保持轻量，通常使用 `shadow-sm`、`shadow-2xs` 或公共组件默认阴影；只有移动端抽屉、弹窗等浮层可使用更明显阴影。
...
- 保持单列、居中和易扫描，主要内容宽度以现有 `max-w-3xl` 为基线。
- 产品标识、用户名、刷新、创建房间和房间列表构成主要层级，不新增营销模块或无关说明区块。
