...
- 发言历史展开时不重复渲染玩家列：发言单元格直接渲染进玩家行内，成为同一 flex 行的右半部分，逐行对齐由 DOM 结构保证，不靠两侧各自复刻行高与间距。
- 展开/收起按钮骑缝放在玩家面板右边框中点，尺寸不小于 `h-8 w-8`，使用 `PanelRightOpen` / `PanelRightClose` 表达面板开合方向，并提供 `aria-label` 与 `aria-expanded`。
