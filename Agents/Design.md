...
- 全站排版采用衬线体（Serif）作为主要视觉基线：页面主体（`body`）默认使用 `var(--font-serif)` 并配合 `antialiased` 抗锯齿优化，延续 Vintage Paper 的复古、典雅质感。标题、游戏阶段说明、对话气泡、系统消息、全局徽章、顶栏信息及操作按钮等默认继承衬线体。
- 衬线字体栈（`--font-serif`）严格遵守西文字体置顶原则，杜绝中文字体自带西文导致的屏显发虚与字形劣化：
...
- 页脚自上而下为玩法指南文字链接行、友情链接文字链接行、外部链接图标行与版本号，居中排列。版本号点击后在弹窗中分标签展示更新日志与提交历史，不在页面主体常驻展开这两块内容。
- 玩法指南行是站内长尾内容页（`/guide/*`）的主要入口，与友情链接同款紧凑文字样式但指向站内，不加外链图标；这一行不要删。
...
- 路由为 `/guide/<slug>`，slug 与正文由 `Client/src/data/Guides.ts` 单一维护，页面组件只负责排版；未收录的 slug 直接回主页，不留空白页。内容来自 `Agents/` 下的规则文档，规则变更必须同步该文件。
- 保持单列阅读形态，内容宽度以 `max-w-3xl` 为基线；顶部沿用大厅页的「返回主页 + 站内标识」头部结构，正文直接排在页面上，不用卡片包裹区块。
- 结构性元素只有：H1（页标题）、H2（小节）、H3（问答）、正文段落与要点列表。正文用全站衬线体 `text-sm leading-7`，只有「更新于」这类元信息取 `text-muted-foreground`。
- 不做入场动效：长文阅读页的动效回答不了「什么导致了它」，按第 8 节原则不加。
- 页尾固定两件事：进入对应游戏的 `Button`（主要操作），以及指向其余指南的内链——内容页之间的内链是它们被收录、传递权重的主要路径。
- 结构化数据同时标注 `Article` 与 `FAQPage`（走 `Seo` 的 `structuredData`，以 `@graph` 承载）；问答区块用 `dl` / `dt` / `dd` 真文本渲染，不做折叠隐藏——隐藏内容对不执行 JS 的爬虫等于不存在。
