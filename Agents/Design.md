...
- 动效令牌统一维护于 `Client/src/lib/motion.ts`，弹性曲线（`spring`）、时长（`duration`）、交互反馈（`pressable` / `pressableStrong` / `tappable`）及编排变体（`listItem` / `listContainer` / `phaseSwap` / `popover` 等）均从该文件取值；不在业务组件内写死时长或 easing。`App.tsx` 顶层已配置 `<MotionConfig reducedMotion="user" />`，系统开启减弱动效时自动跳过所有 framer-motion 动画。
...
- 常规淡入、阶段切换和交互反馈时长以 `@/lib/motion` 中的档位为基线（进场用 `spring.swift`，按压反馈用 `spring.snap`，退出用 `duration.quick`），不在业务组件内写死具体数值；动效幅度应小，不得延迟玩家操作。
- 列表变化使用 `listItem`（缩放淡入）与 `listContainer`（步进节奏）；不对批量列表项使用统一纵向位移（translateY），避免多行同时飘入的感知。侧栏可以使用短距离滑入；加载指示可以持续旋转。
