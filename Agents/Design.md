...
- 玩家身份一律以双字徽章呈现，并统一落在玩家名之前的行首。出题人视角与结算公开的真实身份铺色底，本人的身份预测用同色系虚线描边、不铺底，表达“未确认”。徽章宽度固定，避免各行行首错位。
- 行首徽章只有一套几何：身份、主持、旁观与准备状态共用同一高度、宽度、字重与一层边框，靠颜色而非尺寸区分含义。配色统一为低饱和色底 + 深同色系文字 + 同色边框：饱和实底在暖色纸质底上过于跳脸，对比度应由文字深浅取得，而不是底色艳度。
- 浮层内的选中态例外：`bg-background/95` 之类的浮层底会吃掉半透明色底，因此身份选择器这类浮层内控件的选中档单独用实底。
