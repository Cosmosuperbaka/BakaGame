...
- 桌面端展开/收起按钮骑缝放在玩家面板右边框中点，尺寸不小于 `h-8 w-8`，底色用 `bg-secondary`，不用接近纯白的 `bg-card`。图标用 `ChevronRight` / `ChevronLeft` 指向面板将要移动的方向，并提供 `aria-label` 与 `aria-expanded`。展开后面板已占满整段，按钮须随之内收，否则会落到裁切区外被切掉。
...
- 玩家身份一律以双字徽章呈现，并统一落在玩家名之前的行首。出题人视角与结算公开的真实身份用实底，本人的身份预测用同色系虚线描边表达“未确认”。徽章宽度固定，避免各行行首错位。
- 行首徽章只有一套几何：身份、主持、出局、旁观与准备状态共用同一高度、宽度与字重，靠颜色而非尺寸区分含义。状态徽章同为实底，不用 `bg-*/15` 这类低透明度底色——暖色纸质底上几乎看不出来。
...
- 不添加纯装饰性循环动画、夸张缩放、视差或影响阅读的背景动效。等待类占位（如未提交发言）可使用 `PendingSpeech` 的循环省略号，因为它表达真实的等待状态。
