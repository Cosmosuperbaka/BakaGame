...
const toResult = (row: any): BangumiSubjectSearchResult => ({ id: String(row.id), name: row.name, nameCn: row.name_cn || row.name, year: row.date ? Number(String(row.date).slice(0, 4)) : undefined, rating: row.score || undefined, ratingCount: row.rating_count || undefined, tags: parseList(row.tags), metaTags: parseList(row.meta_tags) });
...
export class LocalBangumiProvider implements BangumiDataProvider {
  ...
  constructor(songPath: string, characterPath: string) {
