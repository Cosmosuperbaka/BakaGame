...
import { randomRoomId } from "@/lib/helpers";
import { backdrop, listItem, selectable, spinner, spring, useOriginTracker } from "@/lib/motion";
import { useSongGuessrStore } from "@/stores/useSongGuessrStore";
import type { SongGuessrRoomSummary } from "@/types";

const phaseLabels: Record<SongGuessrRoomSummary["phase"], string> = {
  waiting: "等待中",
...
export default function SongGuessrPage() {
  ...
  const rooms = useSongGuessrStore((state) => state.rooms);
  ...
  const subscribeLobby = useSongGuessrStore((state) => state.subscribeLobby);
  const setNotice = useSongGuessrStore((state) => state.setNotice);

  const [userName, setUserName] = useState(getSavedUsername);
  const [createOpen, setCreateOpen] = useState(false);
  const [joinTarget, setJoinTarget] = useState<SongGuessrRoomSummary | null>(null);
  ...
  const handleJoinRoom = useCallback(
    async (room: SongGuessrRoomSummary, event: React.MouseEvent<HTMLElement>) => {
